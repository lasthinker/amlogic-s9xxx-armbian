...
- name: Upload Armbian Firmware to Release
  ...
  with:
    ...
    body: |
      This is Armbian firmware for Amlogic S905X
...
- Enter the `cmd` command mode. Enter the `adb connect 192.168.2.137` command (the ip is modified according to your box, and you can check it in the router device connected to the box), If the link is successful, it will display `connected to 192.168.2.137:5555`
